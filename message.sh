...
#$1-文件
record_total_size() {
	echo -e "$TASK_STATE" | grep -w "$1" | awk '{print $2}'
}

#$1-文件
record_loading_size() {
	echo -e "$TASK_STATE" | grep -w "$1" | awk '{print $3}'
}

#$1-文件
record_task_id() {
	echo -e "$TASK_STATE" | grep -w "$1" | awk '{print $4}'
}

#$1-文件
record_down_state() {
	echo -e "$TASK_STATE" | grep -w "$1" | awk '{print $5}'
}

#$1-同步目录
record_done_total() {
	id=$(echo $1 | sed "s#\/#\\\/#g")
	ID=$(echo -e $TASK_STATE | awk 'find==1 && /^\[/ {exit} /\['$id'\]/ {find=1} find==1 && /^[^[]/ && /'$DOWNER_DONE'/ {printf("%s ", $2);}')
	#echo "id=$ID end"
	total=$(echo $ID | wc -w)
...
record_loading_total() {
	id=$(echo $1 | sed "s#\/#\\\/#g")
	ID=$(echo -e $TASK_STATE | awk 'find==1 && /^\[/ {exit} /\['$id'\]/ {find=1} find==1 && /^[^[]/ && /'$DOWNER_LOADING'/ {printf("%s ", $2);}')
	#echo "id=$ID end"
...
add_record() {
	is=$(echo -e $TASK_STATE | grep "\[$2\]")
	if [ "$is" != "" ]; then
		#echo "add_record=$TASK_STATE"
		id=$(echo $2 | sed "s#\/#\\\/#g")
		TASK_STATE=$(echo -e $TASK_STATE | sed "/^\[${id}\]/a ${1} ${3} ${4} ${5} ${6}" | sed -z "s#\n#\\\n#g")
	fi
}

#$1-url
remove_record() {
	line=$(echo -e ${TASK_STATE} | grep -w -n $1 | awk -F':' '{print $1}')
	TASK_STATE=$(echo -e $TASK_STATE | sed "$line d" | sed -z "s#\n#\\\n#g")
...
sub_task_running_total() {
	ID=$(echo -e $TASK_STATE | awk ' /^\[/ && /'$TASK_RUNNING'/ {printf("%s ", $2);}')
	#echo "id=$ID end"
	total=$(echo $ID | wc -w)
...
#无参数
sub_task_done_total() {
	ID=$(echo -e $TASK_STATE | awk ' /^\[/ && /'$TASK_DONE'/ {printf("%s ", $2);}')
	total=$(cat $ID | wc -w)
...
sub_task_all_record_id() {
	id=$(echo $1 | sed "s#\/#\\\/#g")
	list=$(echo -e $TASK_STATE | awk 'find==1 && /^\[/ {exit} /\['$id'\]/ {find=1} find==1 && /^[^[]/ {printf("%s ", $4);}')
...
set_sub_task_state() {
	ID=$(sub_task_id $1)
	line=$(echo -e $TASK_STATE | grep -n -e "^[$1]")
	

	TASK_STATE=$(echo -e $TASK_STATE | sed "$line d" | sed -z "s#\n#\\\n#g")

	echo "pass" #TASK_STATE=$(echo -e $TASK_STATE | sed "/^[${1}]/c [${1}] ${ID} ${2}")
}
...
#$1-taskID $2-状态
set_task_state() {
	echo $TASK_STATE | grep -e "^[$1]"
	if [ $? -eq 0 ]; then
		TASK_STATE=$(sed "/^[${1}]/c [${1}] ${2}")
	fi
}

# $1-保存信息的目录
sync_task_info() {
	#echo -e "info: $TASK_STATE end"
	echo -e "$TASK_STATE" >$1
	sync
}
